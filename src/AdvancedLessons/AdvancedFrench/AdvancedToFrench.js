...
import "./AdvancedToFrench.css";  

function AdvancedToFrench() {
  ...
  const storageKey = "quizProgress_AdvancedFrench";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_AdvancedFrench", JSON.stringify({ score: updatedScore }));

      const highestScore = parseInt(localStorage.getItem("highestScore_AdvancedToFrench"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_AdvancedToFrench", updatedScore);
      }
    }
  };
...
export default AdvancedToFrench;
