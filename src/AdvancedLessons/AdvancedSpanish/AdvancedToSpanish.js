...
function IntermediateToSpanish() {
  ...
  const storageKey = "quizProgress_AdvancedSpanish";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_AdvancedSpanish", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_AdvancedToSpanish"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_AdvancedToSpanish", updatedScore);
      }
    }
  };
