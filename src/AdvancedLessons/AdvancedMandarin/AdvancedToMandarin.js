...
function IntermediateToMandarin() {
  ...
  const storageKey = "quizProgress_AdvancedMandarin";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_AdvancedMandarin", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_AdvancedToMandarin"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_AdvancedToMandarin", updatedScore);
      }
    }
  };
