...
import "./IntroToItalian.css";

function IntroToItalian() {
  ...
  const storageKey = "quizProgress_Italian";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_Italian", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_IntroToItalian"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_IntroToItalian", updatedScore);
      }
    }
  };
