...
import "./IntroToGerman.css";

function IntroToGerman() {
  ...
  const storageKey = "quizProgress_German";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_German", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_IntroToGerman"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_IntroToGerman", updatedScore);
      }
    }
  };
