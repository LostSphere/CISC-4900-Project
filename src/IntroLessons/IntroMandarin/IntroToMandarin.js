...
import "./IntroToMandarin.css";

function IntroToMandarin() {
  ...
  const storageKey = "quizProgress_Mandarin";
  ...
  const handleTextToSpeech = (text) => {
    if ('speechSynthesis' in window) {
      ...
      const setVoiceAndSpeak = () => {
        ...
        if (voices.length > 0) {
          const selectedVoice = voices.find(voice => voice.lang === 'zh-CN') || voices[0];
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_Mandarin", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_IntroToMandarin"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_IntroToMandarin", updatedScore);
      }
    }
  };
...
export default IntroToMandarin;
