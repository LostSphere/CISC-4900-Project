...
function IntroToFrench() {
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_French", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_IntroToFrench"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_IntroToFrench", updatedScore);
      }
    }
  };
