...
import "./GermanStory.css";

function GermanStory() {
  ...
  const storageKey = "quizProgress_GermanRoadTrip";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_GermanRoadTrip", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_GermanRoadTrip"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_GermanRoadTrip", updatedScore);
      }
    }
  };
