...
function SpanishStory() {
  ...
  const storageKey = "quizProgress_SpanishFiesta";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_SpanishFiesta", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_SpanishFiesta"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_SpanishFiesta", updatedScore);
      }
    }
  };
