...
function DebatingMadrid() {
  ...
  const storageKey = "quizProgress_DebatingMadrid";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_DebatingMadrid", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_DebatinginMadrid"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_DebatinginMadrid", updatedScore);
      }
    }
  };
