...
import "./ItalianStory.css"; 

function ExploringRome() {
  ...
  const storageKey = "quizProgress_ExploringRome";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_ExploringRome", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_ExploringRome"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_ExploringRome", updatedScore);
      }
    }
  };
