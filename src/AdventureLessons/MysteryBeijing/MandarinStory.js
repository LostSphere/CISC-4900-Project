...
function MandarinStory() {
  ...
  const storageKey = "quizProgress_MysteryBeijing";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_MysteryBeijing", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_MysteryinBeijing"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_MysteryinBeijing", updatedScore);
      }
    }
  };
