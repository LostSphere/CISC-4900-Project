...
function ParisianCafe() {
  ...
  const storageKey = "quizProgress_ParisianCafe";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_ParisianCafe", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_ParisianCafe"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_ParisianCafe", updatedScore);
      }
    }
  };
