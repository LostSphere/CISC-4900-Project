...
import "./MandarinStory.css"; 

function BusinessShanghai() {
  ...
  const storageKey = "quizProgress_BusinessShanghai";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_BusinessShanghai", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_BusinessinShanghai"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_BusinessinShanghai", updatedScore);
      }
    }
  };
...
export default BusinessShanghai;
