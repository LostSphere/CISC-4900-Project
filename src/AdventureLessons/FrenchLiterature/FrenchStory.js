...
import "./FrenchStory.css";  

function FrenchLiterature() {
  ...
  const storageKey = "quizProgress_FrenchLiterature";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_FrenchLiterature", JSON.stringify({ score: updatedScore }));

      const highestScore = parseInt(localStorage.getItem("highestScore_FrenchLiterature"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_FrenchLiterature", updatedScore);
      }
    }
  };
...
export default FrenchLiterature;