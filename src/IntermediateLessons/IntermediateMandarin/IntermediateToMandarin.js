...
import storyData from "./MandarinData.json";
import quizData from "./MandarinQuizData.json";
import "./IntermediateToMandarin.css"; 

function IntermediateToMandarin() {
  ...
  const storageKey = "quizProgress_IntermediateMandarin";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_IntermediateMandarin", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_IntermediateToMandarin"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_IntermediateToMandarin", updatedScore);
      }
    }
  };
  ...
  if (quizCompleted) {
    return (
      <div className="mandarin-story-container">
...
export default IntermediateToMandarin;
