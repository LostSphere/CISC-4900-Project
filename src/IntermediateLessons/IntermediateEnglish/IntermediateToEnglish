...
import "./IntermediateToEnglish.css"; 

function IntermediateToEnglish() {
  ...
  const storageKey = "quizProgress_Intermediate";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_Intermediate", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_IntermediateToEnglish"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_IntermediateToEnglish", updatedScore);
      }
    }
  };
...
export default IntermediateToEnglish;
