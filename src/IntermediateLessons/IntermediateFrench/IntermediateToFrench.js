...
import "./IntermediateToFrench.css"; 

function IntermediateToFrench() {
  ...
  const storageKey = "quizProgress_IntermediateFrench";
  ...
  const handleTextToSpeech = (text) => {
    if ('speechSynthesis' in window) {
      ...
      const setVoiceAndSpeak = () => {
        ...
        if (voices.length > 0) {
          const selectedVoice = voices.find(voice => voice.lang === 'fr-FR') || voices[0];
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_IntermediateFrench", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_IntermediateToFrench"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_IntermediateToFrench", updatedScore);
      }
    }
  };
...
export default IntermediateToFrench;
