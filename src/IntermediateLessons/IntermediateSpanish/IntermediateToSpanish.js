...
import "./IntermediateToSpanish.css";

function IntermediateToSpanish() {
  ...
  const storageKey = "quizProgress_IntermediateSpanish";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_IntermediateSpanish", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_IntermediateToSpanish"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_IntermediateToSpanish", updatedScore);
      }
    }
  };
...
export default IntermediateToSpanish;
