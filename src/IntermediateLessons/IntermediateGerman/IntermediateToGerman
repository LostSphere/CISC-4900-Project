...
import storyData from "./GermanData.json";
import quizData from "./GermanQuizData.json";
import "./IntermediateToGerman.css";

function IntermediateToGerman() {
  ...
  const storageKey = "quizProgress_IntermediateGerman";
  ...
  const handleQuizAnswer = (selectedOption) => {
    ...
    if (currentQuestionIndex < quizData.length - 1) {
      ...
      localStorage.setItem("quizProgress_IntermediateGerman", JSON.stringify({ score: updatedScore }));
  
      const highestScore = parseInt(localStorage.getItem("highestScore_IntermediateToGerman"), 10) || 0;
      if (updatedScore > highestScore) {
        localStorage.setItem("highestScore_IntermediateToGerman", updatedScore);
      }
    }
  };
  ...
  if (quizCompleted) {
    return (
      <div className="german-story-container">
...
export default IntermediateToGerman;
