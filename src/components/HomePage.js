...
function HomePage() {
  ...
  const [profileImage, setProfileImage] = useState("/images/profileImages/Pig.jpg"); // Default image path
  ...
  useEffect(() => {
    ...
    if (savedProfileImage) {
      setProfileImage(savedProfileImage); // Use saved image from localStorage
    }
  }, [navigate]);
          ...
          <Link to="/settings" className="profile-image">
            {profileImage ? (
              <img
                ...
                onError={() => setProfileImage("/images/profileImages/Pig.jpg")} // Fallback to default image
              />
            ) : (
